-- 1) completed_at em plannings
ALTER TABLE public.plannings ADD COLUMN IF NOT EXISTS completed_at timestamptz;

CREATE OR REPLACE FUNCTION public.handle_planning_completed_at()
RETURNS trigger
LANGUAGE plpgsql
...
DROP TRIGGER IF EXISTS plannings_completed_at_trg ON public.plannings;
CREATE TRIGGER plannings_completed_at_trg
BEFORE INSERT OR UPDATE ON public.plannings
FOR EACH ROW EXECUTE FUNCTION public.handle_planning_completed_at();

UPDATE public.plannings SET completed_at = updated_at WHERE status = 'completed' AND completed_at IS NULL;

-- 2) Base semanal de entregas
...
BEGIN
  IF NOT (public.is_team_member(auth.uid()) OR public.has_role(auth.uid(), 'admin')) THEN
